...
export const DashboardUpcomingEvents: React.FC<
...
> = ({ userId, onGoToAgenda }) => {
    ...
    useEffect(() => {
        ...
        // Calcular fechas: Ayer y Mañana
        const now = new Date()
        const yesterday = new Date(now)
        yesterday.setDate(now.getDate() - 1)
        const tomorrow = new Date(now)
        tomorrow.setDate(now.getDate() + 2) // +2 para incluir mañana completo

        const yesterdayStr = yesterday.toISOString().split('T')[0]
        const tomorrowStr = tomorrow.toISOString().split('T')[0]

        // Consulta: Eventos en este rango
        const q = query(
    ...
    return (
        <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {events.map((ev) => {
                    const isToday =
                        ev.date === new Date().toLocaleDateString('en-CA')
                    return (
                        <div
                            key={ev.id}
                        ...
                        >
                            ...
                            {ev.time && (
                                <div className="text-right">
                                    <div className="bg-slate-100 p-2 rounded-lg text-slate-600 font-mono text-sm font-bold flex items-center gap-1">
