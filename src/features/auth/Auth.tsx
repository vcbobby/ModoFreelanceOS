...
export const AuthView = ({ onLoginSuccess, onBack }: AuthProps) => {
  ...
  const getPlatformName = () => {
    ...
    if (navigator.userAgent.toLowerCase().includes(' electron/')) return 'Windows App';
    return 'Web Browser';
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (viewState === 'register') {
        ...
        setSuccessMsg('¡Listo! Revisa tu correo para restablecer la contraseña.');
        setLoading(false);
        return;
      }
    } catch (err: any) {
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-100 dark:border-slate-700">
        {isE2E && (
          <div className="mb-4">
            <button
              ...
              className="w-full py-2 rounded-lg bg-slate-900 text-white text-sm font-bold"
            >
              Entrar en modo E2E
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label
              htmlFor="email"
              className="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase mb-1 ml-1"
            >
              Email
            </label>
          ...
          {viewState !== 'forgot' && (
            <div>
              <div className="flex justify-between items-center mb-1">
                <label
                  htmlFor="password"
                  className="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase ml-1"
                >
                  Contraseña
                </label>
                {viewState === 'login' && (
          ...
          >
            {loading ? (
              ...
              <>
                {viewState === 'register' && 'Registrarse Gratis'}
                {viewState === 'login' && 'Entrar al Dashboard'}
                {viewState === 'forgot' && 'Enviar enlace de recuperación'}
                {!loading && viewState !== 'forgot' && <ArrowRight className="w-5 h-5" />}
              </>
            )}
          </button>
        </form>
