...
interface NoteCardProps {
  ...
  onPrivacy: (event: React.MouseEvent) => void;
  onClick: () => void;
  onCheck: (lineIndex: number) => void;
}

const getNoteTimestamp = (value: Note['createdAt']) => {
  if (typeof value === 'number') return value;
  return value?.toMillis?.() ?? 0;
};

const COLOR_MAP: Record<string, { bg: string; border: string; gradient: string }> = {
  'bg-yellow-100': {
...
export const NotesView: React.FC<NotesViewProps> = ({ userId, autoOpenAgenda }) => {
  ...
  const [notes, setNotes] = useState<Note[]>(() => (isE2E && userId ? loadE2eNotes() : []));
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      let loadedNotes = snapshot.docs.map((snap) => ({
        id: snap.id,
      ...
      const pinned = loadedNotes
        .filter((n) => n.isPinned)
        .sort((a, b) => getNoteTimestamp(b.createdAt) - getNoteTimestamp(a.createdAt));
      const unpinned = loadedNotes
        .filter((n) => !n.isPinned)
        .sort((a, b) => {
          if (a.order < 9000 && b.order < 9000) return a.order - b.order;
          return getNoteTimestamp(b.createdAt) - getNoteTimestamp(a.createdAt);
        });
  ...
  const handleDragEnd = async (e: React.DragEvent) => {
    ...
    if (dragItem.current === null || dragOverItem.current === null || !userId) return;
    const _notes = [...notes];
  ...
  const handleToggleCheck = async (note: Note, lineIndex: number, isEditingModal = false) => {
    if (!userId) return;
    ...
    if (isE2E) {
      const nextNotes = notes.map((n) => (n.id === note.id ? { ...n, content: newContent } : n));
      setNotes(nextNotes);
      saveE2eNotes(nextNotes);
    } else {
  ...
  const handleUpdateNote = async () => {
    if (editingNote && userId) {
      ...
      try {
        if (isE2E) {
          const nextNotes = notes.map((n) => (n.id === editingNote.id ? editingNote : n));
          setNotes(nextNotes);
          saveE2eNotes(nextNotes);
        } else {
    ...
    <div className="w-full min-h-screen pb-20 px-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Tablero de Notas</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        <div className="w-full lg:hidden flex justify-center">
          <button
            ...
            className={`w-full max-w-lg flex items-center justify-center gap-2 text-white py-2 rounded-lg font-bold text-sm transition-colors mb-4 ${
              agendaOpen ? 'bg-brand-600' : 'bg-slate-900'
            }`}
          >
            <CalIcon className="w-4 h-4" /> {agendaOpen ? 'Volver a las Notas' : 'Ver Agenda'}
          </button>
        </div>
        <div className={`flex-1 w-full min-w-0 ${agendaOpen ? 'hidden lg:block' : 'block'}`}>
          <div
            ...
            >
              ...
              {isInputExpanded && (
                <div className="flex flex-wrap items-center justify-between px-3 py-2 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 gap-4">
                  <div className="flex gap-1">
                    {Object.keys(COLOR_MAP).map((colorKey) => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-full border ${COLOR_MAP[colorKey].border} ${
                          COLOR_MAP[colorKey].bg
                        } ${
                          selectedColor === colorKey ? 'ring-2 ring-offset-1 ring-slate-400' : ''
                        }`}
                      />
                    ))}
                  </div>
                  ...
                  {noteError && <p className="w-full text-xs text-red-600 px-1">{noteError}</p>}
                </div>
              )}
            </form>
          </div>
            ...
            {notes.map((note, index) => (
              <div
                ...
                className={`mb-4 break-inside-avoid inline-block w-full h-fit transition-transform ${
                  !note.isPinned ? 'cursor-grab active:cursor-grabbing' : ''
                }`}
              >
                <NoteCard
                  note={note}
                  ...
                  onCheck={(lineIdx) => handleToggleCheck(note, lineIdx)}
                />
              </div>
            ))}
          </div>
        </div>
        <div className={`w-full lg:w-96 shrink-0 ${!agendaOpen ? 'hidden lg:block' : 'block'}`}>
          ...
          >
              ...
              <div className="flex bg-black/5 dark:bg-white/10 rounded-lg p-1 flex-none mt-2 md:mt-0">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-md transition-colors ${
                    editMode === 'view' ? 'bg-white dark:bg-slate-700 shadow-sm' : 'text-slate-500'
                  }`}
                ...
                <button
                  ...
                  className={`p-1.5 rounded-md transition-colors ${
                    editMode === 'edit' ? 'bg-white dark:bg-slate-700 shadow-sm' : 'text-slate-500'
                  }`}
              ...
              {editMode === 'edit' ? (
                ...
                <div className="text-slate-800 dark:text-slate-200 text-base leading-relaxed">
                  {editingNote.content.split('\n').map((line: string, idx: number) => {
                    ...
                    if (line.trim().startsWith('☐')) {
                      return (
                        <div
                          ...
                          onClick={() => handleToggleCheck(editingNote, idx, true)}
                        >
                          <span className="text-slate-900 font-bold scale-125 mt-0.5">☐</span>
                    ...
                    if (line.trim().startsWith('☑')) {
                      return (
                        <div
                          ...
                          onClick={() => handleToggleCheck(editingNote, idx, true)}
                        >
                          <span className="text-green-800 font-bold scale-125 mt-0.5">☑</span>
                    ...
                    return (
                      <div key={idx} className="min-h-[1.5em] mb-1 whitespace-pre-wrap">
                        {line}
                      </div>
                    );
                  })}
                  {!editingNote.content && <p className="text-slate-400 italic">Escribe algo...</p>}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-white/50 dark:bg-slate-900/50 border-t border-black/5 dark:border-white/10 shrink-0">
                ...
                {Object.keys(COLOR_MAP).map((colorKey) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border ${COLOR_MAP[colorKey].border} ${
                      ...
                      editingNote.color === colorKey ? 'ring-2 ring-slate-400' : ''
                    }`}
                  />
                ))}
              </div>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-slate-700 text-white rounded-lg font-bold hover:bg-black dark:hover:bg-slate-600 transition-colors shadow-lg disabled:opacity-60"
                disabled={isSaving}
              >
...
const NoteCard = ({ note, onDelete, onPin, onPrivacy, onClick, onCheck }: NoteCardProps) => {
  ...
  return (
    ...
    >
      ...
      {note.title && (
        <h3 className="font-bold text-slate-800 dark:text-white mb-2 pr-6">{note.title}</h3>
      )}
      <div
        className={`relative text-sm text-slate-700 dark:text-slate-200 whitespace-pre-wrap leading-relaxed transition-all duration-300 ${
          note.isPrivate && !isHovered ? 'blur-md opacity-50' : 'blur-0 opacity-100'
        } overflow-hidden`}
      >
      ...
      >
        <button
          ...
          className={`p-1.5 rounded hover:bg-white dark:hover:bg-slate-700 ${
            note.isPinned ? 'text-brand-600' : 'text-slate-500 dark:text-slate-400'
          }`}
        >
        ...
        >
          {note.isPrivate ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
        </button>
        <button
          onClick={handleCopy}
...
const AgendaWidget = ({ userId }: { userId?: string }) => {
  ...
  const inputClass =
    'w-full p-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white dark:bg-slate-900 dark:text-white transition-all placeholder:text-slate-400';
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  useEffect(() => {
    if (!userId) return;
    const q = query(collection(db, 'users', userId, 'agenda'), orderBy('date', 'asc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const nextEvents = snapshot.docs.map((docSnap) => ({
        id: docSnap.id,
        ...(docSnap.data() as Omit<AgendaEvent, 'id'>),
      }));
  ...
  const getEventStatus = (dateStr: string, timeStr: string) => {
    ...
    if (diffMins > 0 && diffMins <= 60) {
      return {
        ...
        color:
          'text-orange-600 bg-orange-50 dark:bg-orange-900/30 border-orange-200 dark:border-orange-800',
        animate: true,
      };
    }
    if (diffMins <= 0 && diffMins > -60) {
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden sticky top-6">
      <div className="bg-slate-50 dark:bg-slate-900 p-4 border-b border-slate-100 dark:border-slate-700">
        ...
        <div className="grid grid-cols-7 text-center text-xs gap-1 mb-1">
          {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((d, i) => (
            <span key={i} className="text-slate-400 dark:text-slate-500 font-bold">
              {d}
            </span>
          ))}
        </div>
      ...
      <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar">
        ...
        >
          ...
          <button
            type="submit"
            className="w-full bg-slate-900 dark:bg-slate-700 text-white py-2 rounded-lg text-xs font-bold hover:bg-black dark:hover:bg-slate-600 flex items-center justify-center gap-1 transition-colors"
          >
            <Plus className="w-3 h-3" /> Agregar a la agenda
          </button>
        </form>
        {displayEvents.length === 0 ? (
          <p className="text-center text-xs text-slate-400 py-4">No hay eventos para esta fecha.</p>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  {ev.link && (
                    ...
                    >
                      <Video className="w-3 h-3" /> Unirse a reunión
                    </a>
                  )}
                </div>
              );
