...
export const CopilotWorkspace: React.FC<CopilotWorkspaceProps> = ({ projectId, onBack, userId, onUsage }) => {
  ...
  return (
    <div className="h-auto min-h-[calc(100vh-80px)] lg:h-[calc(100vh-80px)] max-w-7xl mx-auto flex flex-col lg:flex-row gap-4 lg:gap-6 p-4 lg:py-6 lg:pb-20">
      ...
      <div className="w-full h-[600px] flex-none lg:flex-1 lg:h-full bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50 dark:bg-slate-900/50">
           {messages.map((m, i) => {
             ...
             <div key={i} className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[95%] sm:max-w-[85%] rounded-2xl p-4 shadow-sm text-sm ${isUser ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded-tl-none'}`}>
                   {!isUser && i === 0 && <Sparkles className="w-4 h-4 text-indigo-500 mb-2" />}
                   <div className={`prose prose-sm max-w-full overflow-x-auto ${isUser ? 'text-white prose-p:text-white prose-a:text-white' : 'dark:prose-invert prose-p:leading-relaxed text-slate-800 dark:text-slate-100'} [&>pre]:p-3 [&>pre]:rounded-xl`}>
