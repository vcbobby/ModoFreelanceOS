...
export const InvoiceTool: React.FC<InvoiceToolProps> = ({ onUsage, userId }) => {
  ...
  const getE2eKey = (id?: string) => `e2e_invoice_seq_${id || 'anon'}`;
      ...
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <FileText className="w-6 h-6 text-brand-600" /> Generador de Facturas
          </h2>
        ...
        <Card className="p-6 shadow-md space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              {logo && <img src={logo} alt="Logo preview" className="h-12 object-contain" />}
              ...
              {logo && (
                <button onClick={() => setLogo(null)} className="text-red-500 text-xs">
                  Eliminar
                </button>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2 border-b border-slate-100 dark:border-slate-700 pb-4">
              <h3 className="font-bold text-slate-800 dark:text-white">Tus Datos (Emisor)</h3>
              <input
            ...
            <div className="space-y-2">
              <h3 className="font-bold text-slate-800 dark:text-white">Cliente (Receptor)</h3>
              <input
          ...
          <div>
            <h3 className="font-bold text-slate-800 dark:text-white mb-2">Conceptos</h3>
            <div className="space-y-2">
              {items.map((item) => (
                <div
                  key={item.id}
          ...
          <Button onClick={handleDownload} isLoading={isDownloading} className="w-full">
            {isDownloading ? 'Generando PDF...' : `Descargar Factura (3 Créditos)`}
          </Button>
        </Card>
      </div>
              ...
              <table className="w-full mb-8">
                <thead>
                  ...
                  >
                    ...
                    <th className="text-right py-2 font-bold uppercase text-[10px] w-20">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item) => (
