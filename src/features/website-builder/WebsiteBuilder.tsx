...
export const WebsiteBuilder: React.FC<WebsiteBuilderProps> = ({ onUsage, userId }) => {
  ...
  const [activeSection, setActiveTab] = useState<'general' | 'design' | 'projects' | 'cv'>(
    'general'
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isE2E) {
        saveE2eSite(dataToSave);
      } else {
        await setDoc(doc(db, 'users', userId, 'portfolio', 'site_config'), dataToSave);
  ...
  // Galería
  const addToGallery = (idx: number, url: string, type: string, name: string) => {
    const p = [...(siteData.projects || [])];
  ...
  // Documentos
  const addToDocs = (idx: number, url: string, type: string, name: string) => {
    const p = [...(siteData.projects || [])];
      ...
      {docModal.isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl w-full max-w-sm shadow-2xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-bold mb-4 dark:text-white text-lg">Nombre del Documento</h3>
            <input
              autoFocus
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <Button onClick={confirmDocUpload}>Subir Archivo</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Monitor className="w-8 h-8 text-brand-600" /> Web Builder PRO
          </h2>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <Button
            variant="outline"
            onClick={() =>
              window.open(`${window.location.origin}/p/${siteData.slug || userId}`, '_blank')
            }
              ...
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 md:p-6 rounded-xl border border-blue-100 dark:border-blue-800">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <input
                      ...
                      value={siteData.slug}
                      onChange={(e) =>
                        update('slug', e.target.value.replace(/\s+/g, '-').toLowerCase())
                      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="w-full min-w-0">
                    <label className={labelClass} htmlFor="website-name">
                      Nombre Visible
                    </label>
                    <input
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <input
                      ...
                      onChange={(e) => update('twitch', e.target.value)}
                      placeholder="Twitch URL"
                    />
                    <input
                      className={inputClass}
                      value={siteData.kick}
                      onChange={(e) => update('kick', e.target.value)}
                      placeholder="Kick URL"
                    />
                    <input
                      className={inputClass}
                      value={siteData.patreon}
                      onChange={(e) => update('patreon', e.target.value)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input
                      ...
                      onChange={(e) => update('upwork', e.target.value)}
                    ...
                    <input
                      ...
                      onChange={(e) => update('workana', e.target.value)}
                      placeholder="Perfil Workana"
                    />
                    <input
                      className={inputClass}
                      value={siteData.fiverr}
                      onChange={(e) => update('fiverr', e.target.value)}
                ...
                <div>
                  <label className={`${labelClass} mb-2`}>Foto de Perfil</label>
                  <div className="flex items-center gap-4 p-4 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <label className="cursor-pointer bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-black transition-colors">
                      {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Subir Foto'}
                      <input
                        type="file"
                        onChange={(e) => handleFileUpload(e, (url) => update('photo', url))}
                        className="hidden"
                        accept="image/*"
                        disabled={uploading}
                      />
            ...
            {activeSection === 'projects' && (
              <div className="space-y-8 animate-in fade-in">
                <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Portafolio</h3>
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                          <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                            <p className={`${labelClass} mb-2`}>Portada</p>
                            <label className="cursor-pointer bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 px-3 py-2 rounded text-xs font-bold block text-center transition-colors">
                              {uploading ? '...' : 'Subir Portada'}
                              <input
                                type="file"
                                onChange={(e) =>
                                  handleFileUpload(e, (url) => updateProject(i, 'cover', url))
                                }
                                className="hidden"
                                accept="image/*"
                                disabled={uploading}
                          ...
                          <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                            <p className={`${labelClass} mb-2`}>Galería (Fotos/Video)</p>
                          ...
                          <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                            <p className={`${labelClass} mb-2`}>Documentos</p>
                            <label className="cursor-pointer bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 px-3 py-2 rounded text-xs font-bold block text-center transition-colors mb-2">
                              ...
                              <input
                                type="file"
                                accept=".pdf,.doc,.docx"
                                onChange={(e) => handleDocSelect(e, i)}
                                className="hidden"
                                disabled={uploading}
                              />
                            </label>
              ...
              <div className="space-y-8 animate-in fade-in">
                ...
                <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold dark:text-white">Historial Laboral</h3>
                    <Button size="sm" onClick={addExp} variant="secondary">
                ...
                <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <Button size="sm" onClick={addEdu} variant="secondary">
                      <Plus className="w-4 h-4 mr-1" /> Agregar
                    </Button>
                  </div>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            {/* EXPERIENCIA Y EDUCACIÓN (En bloque para que no se rompan feo) */}
