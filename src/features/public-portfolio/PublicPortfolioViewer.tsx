...
const Socials = ({ data, dark = false }: { data: PortfolioData; dark?: boolean }) => {
  const links = [
    {
      url: data.linkedin,
...
const FreelancePlatforms = ({ data, dark = false }: { data: PortfolioData; dark?: boolean }) => {
  const links = [
    {
      url: data.upwork,
...
// --- FOOTER ---
const Footer = ({ name: _name, isDark }: { name?: string; isDark?: boolean }) => (
  <footer
  ...
  >
    <p className="font-medium">
      &copy; {new Date().getFullYear()} ModoFreelanceOS. Todos los derechos reservados.
    </p>
    <a
...
const SharedSections = ({ data, accent: _accent, openProject, gridMode, isDark }: LayoutProps) => (
  <div className="space-y-24 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 w-full">
    {data.projects && data.projects.length > 0 && (
      <section>
        ...
        <div
          className={`grid gap-8 ${gridMode ? 'md:grid-cols-2 lg:grid-cols-3' : 'md:grid-cols-2'}`}
        >
            ...
            >
              <div
                className={`aspect-[4/3] relative overflow-hidden ${
                  isDark ? 'bg-gray-800' : 'bg-gray-100 dark:bg-gray-800'
                }`}
              >
                {p.cover ? (
                  <img
        ...
        <section>
          ...
          <div
            className={`space-y-10 border-l-2 pl-8 ml-2 ${
              isDark ? 'border-white/20' : 'border-black/10 dark:border-white/20'
            }`}
          >
...
const ClassicLayout = ({ data, accent, openProject }: LayoutProps) => (
...
const SplitLayout = ({ data, accent, openProject }: LayoutProps) => (
  ...
  "
    ...
    [&_svg]:text-slate-700 dark:[&_svg]:text-white"
    >
      <SharedSections data={data} accent={accent} openProject={openProject} isDark={true} />
...
const GridLayout = ({ data, accent, openProject }: LayoutProps) => (
  <div className="min-h-screen pb-10">
    ...
    <div className="container mx-auto px-6 max-w-7xl">
      <SharedSections data={data} accent={accent} openProject={openProject} gridMode={true} />
    </div>
    <Footer name={data.name} />
  </div>
);
...
const TerminalLayout = ({ data, accent, openProject }: LayoutProps) => (
  <div className="bg-[#0d1117] text-[#00ff41] min-h-screen font-mono p-4 flex flex-col">
    <div className="max-w-4xl mx-auto border border-[#30363d] rounded-lg bg-[#010409] p-6 md:p-10 shadow-2xl mt-10 w-full flex-1">
      ...
      <p className="mb-4 text-gray-400">Last login: {new Date().toDateString()}</p>
      <h1 className="text-4xl font-bold mb-6 text-white">{data.name}</h1>
      <p className="text-xl mb-6 text-green-300">{data.role}</p>
      <p className="opacity-80 mb-8 max-w-2xl text-gray-300">{data.bio}</p>
      <ContactActions data={data} accent={accent} style="terminal" />
      <div className="mt-6 flex flex-col items-start gap-2">
        <Socials data={data} dark={true} />
        <FreelancePlatforms data={data} dark={true} />
...
const NeoLayout = ({ data, accent, openProject }: LayoutProps) => (
...
const GlassLayout = ({ data, accent, openProject }: LayoutProps) => (
    ...
    <div className="container mx-auto px-6 py-20 relative z-10">
      <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 md:p-16 shadow-2xl">
        <div className="flex flex-col md:flex-row gap-10 items-center mb-16 border-b border-white/10 pb-10">
          ...
          <div>
            <h1 className="text-5xl font-bold mb-2 text-white">{data.name}</h1>
            <p className="text-2xl opacity-80 mb-4 font-light text-indigo-200">{data.role}</p>
            <p className="opacity-90 leading-relaxed max-w-lg text-white">{data.bio}</p>
...
const MagazineLayout = ({ data, accent, openProject }: LayoutProps) => (
    ...
    <header className="border-b-4 border-black dark:border-stone-100 py-12 px-6">
      <div className="container mx-auto flex flex-col items-center">
        ...
        <div className="flex items-center gap-4 mt-6 w-full max-w-4xl">
          ...
          <span className="font-sans font-bold uppercase tracking-widest text-sm">{data.role}</span>
          <div className="h-px bg-black dark:bg-stone-100 flex-1"></div>
        </div>
      </div>
    </header>
...
const BentoLayout = ({ data, accent, openProject }: LayoutProps) => (
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-sm flex flex-col md:flex-row items-center gap-8 border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex-1 text-center md:text-left">
          <h1 className="text-4xl font-bold mb-2 text-slate-900 dark:text-white">{data.name}</h1>
          <p className="text-xl text-slate-500 dark:text-slate-400 mb-4">{data.role}</p>
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">{data.bio}</p>
      ...
      <div className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-3xl p-8 shadow-sm flex flex-col justify-center items-center text-center">
        <div className="text-6xl font-black mb-2">{data.projects?.length || 0}</div>
        <div className="text-sm uppercase tracking-widest opacity-60">Proyectos Realizados</div>
      </div>
    ...
    >
      <div className="container mx-auto grid md:grid-cols-2 gap-20">
        <div className="sticky top-20 h-fit">
          ...
          <p className="text-2xl font-bold mb-8 text-slate-800 dark:text-slate-200">{data.role}</p>
          <p className="text-lg max-w-sm mb-10 text-slate-600 dark:text-slate-400">{data.bio}</p>
...
const StudioLayout = ({ data, accent, openProject }: LayoutProps) => (
  <div className="bg-zinc-900 text-white min-h-screen font-sans">
    <div className="container mx-auto px-6 py-32 grid md:grid-cols-2 gap-20">
      <div>
        <h1 className="text-6xl font-light mb-6 tracking-wide">{data.name}</h1>
        <p className="text-zinc-400 text-xl mb-12">{data.role}</p>
        <div className="border-l-2 border-white/20 pl-6 py-2 mb-12">
          <p className="text-lg leading-relaxed text-zinc-300">{data.bio}</p>
        </div>
        <div className="flex flex-col items-start">
          <ContactActions data={data} accent={accent} />
          <div className="mt-6 flex flex-col gap-2">
...
const VibrantLayout = ({ data, accent: _accent, openProject }: LayoutProps) => (
  <div className="min-h-screen bg-indigo-900 text-indigo-50 font-sans">
    <div className="bg-indigo-600 p-16 md:p-32 text-center rounded-b-[3rem] shadow-2xl mb-20 relative overflow-hidden">
      <div className="relative z-10">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white mb-4">{data.name}</h1>
    ...
    <div className="container mx-auto px-6 max-w-5xl pb-20">
      <SharedSections data={data} accent="#fbbf24" openProject={openProject} isDark={true} />
    </div>
    <Footer name={data.name} isDark={true} />
  </div>
);

const PastelLayout = ({ data, accent: _accent, openProject }: LayoutProps) => (
  <div className="min-h-screen bg-rose-50 dark:bg-rose-950 text-rose-900 dark:text-rose-100 font-sans p-6">
    <div className="max-w-5xl mx-auto bg-white dark:bg-rose-900/30 rounded-[2rem] p-10 md:p-20 shadow-xl mt-10 border border-rose-100 dark:border-rose-900">
      <header className="text-center mb-16">
        ...
        <p className="max-w-xl mx-auto text-slate-500 dark:text-rose-200/70">{data.bio}</p>
        <div className="mt-8 flex flex-col items-center">
      ...
      <SharedSections data={data} accent="#f43f5e" openProject={openProject} />
    </div>
    <Footer name={data.name} />
  </div>
);

const NatureLayout = ({ data, accent: _accent, openProject }: LayoutProps) => (
  <div className="min-h-screen bg-[#f1f8f5] dark:bg-[#0a1410] text-[#1a2f26] dark:text-[#e0f2eb] font-sans transition-colors">
    <header className="container mx-auto px-6 py-20 grid md:grid-cols-2 items-center">
      <div className="order-2 md:order-1">
        <h1 className="text-6xl font-bold mb-4 text-green-900 dark:text-green-300">{data.name}</h1>
        <div className="h-2 w-20 bg-green-500 mb-6 rounded-full"></div>
        <p className="text-2xl mb-6 text-green-700 dark:text-green-400">{data.role}</p>
    ...
    <div className="container mx-auto px-6 max-w-6xl pb-20">
      <SharedSections data={data} accent="#22c55e" openProject={openProject} />
    </div>
    <Footer name={data.name} />
  </div>
);
...
export const PublicPortfolioViewer = ({ userId }: { userId: string }) => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        const docRef = doc(db, 'users', userId, 'portfolio', 'site_config');
  ...
  const LayoutComponent = layouts[data.layoutId || ''] || ClassicLayout;
    ...
    <div className={isDark ? 'dark' : ''}>
      ...
      >
        ...
        <LayoutComponent
          data={data}
          accent={accent}
          openProject={(project) => setSelectedProject(project)}
        />
                  ...
                  <div className="w-full md:w-5/12 bg-white dark:bg-slate-900 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-800 flex flex-col md:h-full">
                    ...
                    <div className="p-8 md:p-10 flex-1 md:overflow-y-auto no-scrollbar">
                      ...
                      <div className="flex flex-wrap gap-2 mb-8">
                        {selectedProject.tags?.split(',').map((t: string, idx: number) => (
                          <span
                            key={idx}
                        ...
                        <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800">
                          ...
                          <div className="space-y-3">
                            {selectedProject.documents.map((doc, i) => (
                              <button
                                key={i}
                                onClick={() => forceDownload(doc.url, doc.name)}
                              ...
                              >
                                ...
                                <div className="flex-1 overflow-hidden">
                                  ...
                                  <p className="text-[10px] opacity-50">Clic para descargar</p>
                                </div>
