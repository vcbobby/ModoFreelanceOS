...
export const DashboardPinnedNotes: React.FC<DashboardPinnedNotesProps> = ({
  ...
  useEffect(() => {
    if (!userId) return;
    const q = query(collection(db, 'users', userId, 'notes'), where('isPinned', '==', true));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setPinnedNotes(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });
    return () => unsubscribe();
  }, [userId]);
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {pinnedNotes.map((note) => {
          const borderColor = note.color.replace('bg-', 'border-').replace('100', '200');
            ...
            <div className="p-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
              ...
              <div
                className={`text-slate-800 text-base leading-relaxed transition-all duration-300 ${
                  selectedNote.isPrivate ? 'blur-md hover:blur-0 select-none' : ''
                }`}
              >
                {selectedNote.content.split('\n').map((line: string, idx: number) => {
                  ...
                  if (line.trim().startsWith('☐')) {
                    return (
                      <div
                        ...
                        onClick={() => handleToggleCheck(selectedNote, idx)}
                      >
                  ...
                  // TEXTO NORMAL
                  return (
                    <div key={idx} className="min-h-[1.5em] mb-1">
                      {line}
                    </div>
                  );
                })}
