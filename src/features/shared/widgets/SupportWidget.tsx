...
import { LegalModal } from '@features/shared/ui';
import { TERMS_AND_CONDITIONS, PRIVACY_POLICY } from '@/data/legalTexts';

export const SupportWidget = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'faq' | 'support' | 'legal'>('faq');
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-80 sm:w-96 mb-4 overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-200 flex flex-col max-h-[400px]">
          ...
          <div className="bg-slate-900 p-4 flex justify-between items-center text-white shrink-0">
            <h3 className="font-bold flex items-center gap-2">
              <HelpCircle className="w-5 h-5 text-brand-400" /> Centro de Ayuda
            </h3>
            <button
            ...
            {activeTab === 'support' && (
              <div className="space-y-4">
                <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg border border-green-100 dark:border-green-800 text-sm text-green-800 dark:text-green-300 mb-4">
                  <p>Estamos disponibles de Lunes a Viernes para ayudarte.</p>
                </div>
              ...
              <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
                <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-700">
                  ...
                  <p className="text-xs leading-relaxed mb-2">
                    Al usar ModoFreelanceOS, aceptas usar las herramientas de manera ética. No nos
                    hacemos responsables por el uso indebido de las propuestas generadas...
                  </p>
                  <button
                    onClick={() => openLegal('terms')}
                    className="text-brand-600 dark:text-brand-400 text-xs font-bold hover:underline flex items-center gap-1"
                  >
                    Leer documento completo <ExternalLink className="w-3 h-3" />
                  </button>
                </div>
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-700">
                  ...
                  <p className="text-xs leading-relaxed mb-2">
                    Respetamos tu privacidad. Tus datos de facturación y logos se guardan de forma
                    segura y no se comparten con terceros. Solo tú tienes acceso a tu historial...
                  </p>
                  <button
...
const FAQItem = ({ question, answer }: { question: string; answer: string }) => {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
      ...
      >
        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{question}</span>
        {isOpen ? (
