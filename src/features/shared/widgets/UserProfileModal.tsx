...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
    ...
    const TabButton = ({ id, label, icon: Icon }: { id: Tab; label: string; icon: any }) => (
        <button
            ...
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 text-sm font-medium transition-colors border-b-2 ${activeTab === id
    ...
    return (
        <BaseModal isOpen={isOpen} onClose={onClose} title="Configuración de Cuenta">
            <div className="flex flex-col gap-6 min-h-[400px]">
                ...
                {msg && (
                    <div
                        className={`p-3 rounded-lg text-sm text-center ${msg.type === 'success'
                    ...
                    {activeTab === 'security' && (
                        <div className="space-y-4 animate-in fade-in duration-300">
                            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg flex gap-3 text-sm text-yellow-700 dark:text-yellow-300">
                                ...
                                <p>Si cambias tu contraseña, es posible que debas iniciar sesión nuevamente en todos tus dispositivos.</p>
                            </div>
                            ...
                            <div className="pt-4">
                                ...
                                >
                                    {loading ? 'Actualizando...' : <><ShieldCheck className="w-4 h-4" /> Actualizar Contraseña</>}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'subscription' && (
                        <div className="space-y-6 animate-in fade-in duration-300 text-center">
                            <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
                                <h4 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Estado del Plan</h4>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                    ...
                                    <div className="text-xl font-bold text-slate-800 dark:text-white">
                                        {userState.credits} <span className="text-xs font-normal text-slate-400">/ {userState.isSubscribed ? 'mes' : 'semana'}</span>
                                    </div>
                                </div>
                                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                    ...
                                    <div className="text-xl font-bold text-slate-800 dark:text-white">
                                        {userState.isSubscribed
                                            ? (userState.subscriptionEnd ? new Date(userState.subscriptionEnd).toLocaleDateString() : 'N/A')
                                            : (userState.nextReset ? new Date(userState.nextReset).toLocaleDateString() : 'Pronto')
                                        }
                                    </div>
                                </div>
                            </div>
                            ...
                            {!userState.isSubscribed && (
                                <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center justify-center gap-2 mb-4 text-slate-400">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">O compra solo créditos</span>
                                    </div>
                                    <div className="grid grid-cols-1 gap-3">
                                        {CREDIT_PACKAGES.map((pkg) => (
                                            <div key={pkg.credits} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="text-left">
                                                        <div className="text-sm font-bold text-slate-800 dark:text-white">{pkg.credits} Créditos</div>
                                                        <div className="text-[10px] text-slate-500 line-through">{pkg.oldPrice}</div>
                                                    </div>
                                                </div>
                                                <button
