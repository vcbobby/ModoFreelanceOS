...
import { collection, addDoc, doc, getDoc, setDoc } from 'firebase/firestore' // Agregamos doc, getDoc, setDoc
...
export const InvoiceTool: React.FC<InvoiceToolProps> = ({
    ...
    const [invoiceSequence, setInvoiceSequence] = useState(1) // Contador numérico interno
    ...
    const [items, setItems] = useState([
        { id: 1, desc: 'Servicios de Desarrollo Web', qty: 1, price: 100 },
    ])
    const [taxRate, setTaxRate] = useState(0)
    const [notes, setNotes] = useState(
        'Gracias por su confianza. Pago a 30 días.'
    )
    ...
    // --- 1. CARGAR DATOS Y SECUENCIA AL INICIAR ---
    useEffect(() => {
        const fetchInvoiceData = async () => {
            if (!userId) return

            try {
                // Buscamos datos del usuario (secuencia y perfil si quisiéramos auto-rellenar el emisor)
                const userDoc = await getDoc(doc(db, 'users', userId))

                if (userDoc.exists()) {
                    const data = userDoc.data()

                    // Si existe una secuencia guardada, la usamos
                    if (data.lastInvoiceSequence) {
                        const nextNum = data.lastInvoiceSequence + 1
                        setInvoiceSequence(nextNum)
                        // Formateamos a 3 dígitos (ej: 005)
                        setInvoiceNumber(nextNum.toString().padStart(3, '0'))
                    }
                }
            } catch (error) {
                console.error('Error cargando secuencia de factura', error)
            }
        }
    ...
    const handleDownload = async () => {
        ...
        if (userId) {
            try {
                ...
                // B) ACTUALIZAR LA SECUENCIA EN LA BASE DE DATOS (NUEVO)
                // Guardamos el número actual como el último usado
                await setDoc(
                    doc(db, 'users', userId),
                    {
                ...
                // Incrementamos localmente para la próxima sin recargar
                const nextOne = invoiceSequence + 1
                setInvoiceSequence(nextOne)
                // Opcional: Si quieres que el input cambie inmediatamente a la siguiente:
    ...
    return (
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-slate-600 mt-1">
                        Crea facturas profesionales y descárgalas en PDF.
                ...
                <Card className="p-6 shadow-md space-y-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    // Intentamos mantener la secuencia numérica si el usuario escribe un número
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            />
                            <textarea
                                placeholder="Dirección / ID Legal"
            ...
            <div className="bg-slate-200 p-4 md:p-8 rounded-xl overflow-auto flex justify-center sticky top-6">
                <div
                    id="invoice-preview"
                    ref={invoiceRef}
                    className="bg-white shadow-2xl w-[210mm] min-h-[297mm] p-[15mm] text-slate-800 flex flex-col justify-between"
                    style={{ fontSize: '14px' }}
                >
                    {/* Header */}
                    <div>
                        <div className="flex justify-between items-start mb-10">
                            <div>
                                {logo ? (
                                    ...
                                    <h1 className="text-3xl font-bold text-brand-600 uppercase tracking-widest">
                                        Factura
                                    </h1>
                                )}
                            ...
                            <div className="text-right">
                                <h2 className="text-4xl font-light text-slate-200">
                                    INVOICE
                                </h2>
                                <div className="mt-4">
                        ...
                        <table className="w-full mb-8">
                            <thead>
                                <tr className="border-b-2 border-slate-800">
                                    <th className="text-left py-2 font-bold uppercase text-xs">
                                        Descripción
                                    </th>
                                    <th className="text-right py-2 font-bold uppercase text-xs w-20">
                                        Cant.
                                    </th>
                                    <th className="text-right py-2 font-bold uppercase text-xs w-24">
                                        Precio
                                    </th>
                                    <th className="text-right py-2 font-bold uppercase text-xs w-24">
                                        Total
                        ...
                        <div className="flex justify-end mb-12">
                            <div className="w-64 space-y-2">
                                ...
                                <div className="flex justify-between font-bold text-xl text-slate-900 border-t-2 border-slate-800 pt-2">
                                    <span>Total:</span>
                                    <span>
                                        {currency}
                                        {total.toFixed(2)}
